...
namespace PlayGen.ITAlert.Unity.Settings
{
	public class SettingCreation : MonoBehaviour
	{
		...
		public Dropdown Resolution(int minWidth, int minHeight, Resolution[] newResolutions = null,
			bool layoutHorizontal = true, string title = "Resolution", bool showOnMobile = false)
		{
			...
			var resolutionStrings =
				resolutions.OrderByDescending(res => res.width)
					.ThenByDescending(res => res.height)
					.Select(res => res.width + " x " + res.height)
					.ToList();
			newObj.AddOptions(resolutionStrings);
		...
		public Dropdown Quality(bool layoutHorizontal = true, string title = "Quality", bool showOnMobile = false)
		{
			...
			newObj.AddOptions(qualities);
			newObj.value = QualitySettings.GetQualityLevel();
			return newObj;
		}

		public Toggle FullScreen(bool layoutHorizontal = true, string title = "Full Screen", bool showOnMobile = false)
		{
		...
		public T Custom<T>(string label, SettingObjectType type, bool showOnMobile, bool layoutHorizontal = true)
			where T : UIBehaviour
		{
		...
		private GameObject Create(SettingType setting, bool layoutHorizontal)
		{
			if (setting.ObjectType == SettingObjectType.Button)
			{
				...
				{
					button.GetComponentInChildren<Text>().text = setting.Title;
				}
