...
namespace PlayGen.ITAlert.Unity.GameStates.RoomSubStates
{
	public class StateControllerFactory
	{
		...
		private LobbyState CreateLobbyState(Client photonClient, VoiceController voiceController)
		{
			...
			var initializingTransition = new OnMessageTransition(photonClient, Channel.GameState, typeof(InitializingMessage), InitializingState.StateName);
		...
		private PausedState CreatePausedState(Client photonClient)
		{
			...
			var onFeedbackStateSyncTransition = new OnMessageTransition(photonClient, Channel.GameState, typeof(FeedbackMessage), FeedbackState.StateName);
		...
		private InitializingState CreateInitializingState(Client photonClient)
		{
			...
			var onPlayingStateSyncTransition = new OnMessageTransition(photonClient, Channel.GameState, typeof(PlayingMessage), PlayingState.StateName);
		...
		private FeedbackState CreateFeedbackState(Client photonClient)
		{
			...
			var onLobbyStateSyncTransition = new OnMessageTransition(photonClient, Channel.GameState, typeof(LobbyMessage), LobbyState.StateName);
