...
namespace PlayGen.ITAlert.Unity.Network.Behaviours
{
	...
	public class SubsystemBehaviour : EntityBehaviour<SubsystemState>
	{
		...
		public BoxCollider2D DropCollider { get; private set; }
		...
		{
			get { return EntityState.LogicalId; }
		}

		public bool HasActiveItem
		{
			get { return EntityState.HasActiveItem; }
		}
		...
		{
			transform.position = new Vector3(UIConstants.NetworkOffset.x + (UIConstants.SubsystemSpacing.x * EntityState.X),
				UIConstants.NetworkOffset.y + (UIConstants.SubsystemSpacing.y * EntityState.Y));
		...
		private float GetOffsetPositionAlongSquare(int pathPoint, float scale)
		{
			...
			var offsetPositionAlong = (progressAlongSide + halfScale) % (SquareSideCount * scale);
				// Offsets the position so that entry points are half way along each square side.
