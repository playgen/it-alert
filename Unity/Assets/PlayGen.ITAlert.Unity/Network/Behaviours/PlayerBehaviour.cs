...
namespace PlayGen.ITAlert.Unity.Network.Behaviours
{
	...
	public class PlayerBehaviour : EntityBehaviour<PlayerState>
	{
		...
		{
			get { return EntityState.InventoryItem; }
		}
		...
		[SerializeField] private GameObject _decorator;
		...
		protected override void OnUpdatedState()
		{
			...
			if (_historicPositions.Count == PositionHistory)
			{
				...
				ManageInventory(_historicPositions.Count < InventoryPositionHistory
					? _historicPositions.Last()
					: _historicPositions.ToArray()[InventoryPositionHistory - 1]);
			}
			DrawTrail();
		}
