...
namespace PlayGen.ITAlert.Unity.Network
{
	...
	public class Director : MonoBehaviour
	{
		...
		{
			get { return Simulation != null ? Simulation.Rules : new SimulationRules(); }
		}
		...
		private static Simulation.Simulation InitializeTestSimulation()
		{
			var width = 6;
			var height = 3;
		...
		public static void Initialize(Simulation.Simulation simulation, int playerServerId)
		{
			...
			// center graph 
			UIConstants.NetworkOffset -= new Vector2((float) Simulation.GraphSize.X / 2 * UIConstants.SubsystemSpacing.x,
				(float) Simulation.GraphSize.Y / 2 * UIConstants.SubsystemSpacing.y);
		...
		{
			var players =
				Entities.Values.Where(e => e.Type == EntityType.Player).Select(e => e.EntityBehaviour as PlayerBehaviour).ToArray();
		...
		public static void UpdateSimulation(Simulation.Simulation simulation)
		{
		...
		public static void Finalise(Simulation.Simulation simulation)
		{
			Simulation = simulation;
