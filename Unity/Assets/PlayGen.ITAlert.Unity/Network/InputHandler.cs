...
namespace PlayGen.ITAlert.Unity.Network
{
	...
	public class InputHandler : MonoBehaviour
	{
		...
		private Vector2 _defaultDragBounds = new Vector2(-100f, -100f);
			// we can use a negative number as the lowest value is 0,0
				...
				if (PlayerOwnsItem(item))
				{
					var releaseHit =
						Physics2D.RaycastAll(Camera.main.ScreenToWorldPoint(Input.mousePosition), Vector2.zero)
							.SingleOrDefault(d => d.collider.tag.Equals(Tags.Subsystem));
